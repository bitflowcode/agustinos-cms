// api/upload-audio.js - Proxy con fetch nativo (compatible con Vercel)
...
export default async function handler(req, res) {
  ...
  try {
    console.log('[UPLOAD] Proxying audio upload with fetch...');
    console.log('Content-Type:', req.headers['content-type']);
    console.log('Authorization:', req.headers.authorization ? 'Present' : 'Missing');
    console.log('Content-Length:', req.headers['content-length']);

    // Verificar tamaño (10MB máximo como en el backend)
    ...
    const response = await fetch('http://49.12.239.163:3002/api/upload/audio', {
      method: 'POST',
